...
<body>
    ...
    <c:if test="${not empty tekstPesme}">
        ...
        <p style="font-size: 18px"><strong>Dodao korisnik:</strong> 
		    ...
		    <p style="font-size: 18px"><strong><br><br>Prosečna ocena:</strong> 
            <c:choose>
        ...
        <c:if test="${ulogovaniKorisnik != null && ulogovaniKorisnik.id != tekstPesme.korisnik.id}">
	        <form action="/oceniTekst" method="POST">
	            ...
	            <button type="submit">Oceni</button><br><br>
	        </form>
        </c:if>
		
		<c:if test="${ulogovaniKorisnik != null && !jeAutor}">
		    <form action="${jeOmiljen ? '/ukloniIzOmiljenih' : '/dodajUOmiljene'}" method="POST" style="display: inline;">
		        <input type="hidden" name="tekstId" value="${tekstPesme.id}">
		        <button type="submit" title="${jeOmiljen ? 'Ukloni iz omiljenih' : 'Dodaj u omiljene'}" 
        ...
        <c:if test="${isAdmin}">
             <c:choose>
                <c:when test="${tekstPesme.verifikovan}">
                    <p><strong><br><br>Tekst je verifikovan</strong> ✔️</p>
                </c:when>
                <c:otherwise>
                    <br><br><form action="/verifikacija" method="GET">
            			<input type="hidden" name="tekstId" value="${tekstPesme.id}">
